...
@endif
                  <form id="form-register" class="form" action="{{ route('members.store') }}" method="post">
                    @csrf
                    <!-- 会員ID -->
                    <div class="form-group">
                      <label for="disp-id">会員ID</label>
@if ($errors->has('disp_id'))
                      <input class="form-control validate-error" id="disp-id" type="text" name="disp_id" value="{{ old('disp_id') }}" placeholder="A002000" aria-describedby="disp-id-error" aria-invalid="true">
                      <span id="disp_id-error" class="validate-error">@foreach ($errors->get('disp_id') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="disp-id" type="text" name="disp_id" value="{{ old('disp_id') }}" placeholder="A002000">
@endif
                      <p class="help-block fz-12">※空欄の場合は自動設定されます</p>
                    </div>
                    <!-- ニックネーム -->
                    <div class="form-group">
                      <label for="nickname">ニックネーム</label>
@if ($errors->has('nickname'))
                      <input class="form-control validate-error" id="nickname" type="text" name="nickname" value="{{ old('nickname') }}" placeholder="nickname" aria-describedby="nickname-error" aria-invalid="true">
                      <span id="nickname-error" class="validate-error">@foreach ($errors->get('nickname') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="nickname" type="text" name="nickname" value="{{ old('nickname') }}" placeholder="nickname">
@endif
...
@if ($errors->has('company'))
                      <input class="form-control validate-error" id="company" type="text" name="company" value="{{ old('company') }}" placeholder="株式会社 カンパニー">
                      <span id="company-error" class="validate-error">@foreach ($errors->get('company') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="company" type="text" name="company" value="{{ old('company') }}" placeholder="株式会社 カンパニー" aria-describedby="company-error" aria-invalid="true">
@endif
                    </div>
                    <!-- 部署名 -->
                    <div class="form-group">
...
@if ($errors->has('department'))
                      <input class="form-control validate-error" id="department" type="text" name="department" value="{{ old('department') }}" placeholder="営業部" aria-describedby="department-error" aria-invalid="true">
                      <span id="department-error" class="validate-error">@foreach ($errors->get('department') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="department" type="text" name="department" value="{{ old('department') }}" placeholder="営業部">
@endif
...
@if ($errors->has('position'))
                      <input class="form-control validate-error" id="position" type="text" name="position" value="{{ old('position') }}" placeholder="課長" aria-describedby="position-error" aria-invalid="true">
                      <span id="position-error" class="validate-error">@foreach ($errors->get('position') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="position" type="text" name="position" value="{{ old('position') }}" placeholder="課長">
@endif
                    </div>
                    <!-- 姓 -->
                    <div class="form-group">
...
@if ($errors->has('lname'))
                      <input class="form-control validate-error" id="lname" type="text" name="lname" value="{{ old('lname') }}" placeholder="姓" aria-describedby="lname-error" aria-invalid="true">
                      <span id="lname-error" class="validate-error">@foreach ($errors->get('lname') as $message){{ $message }}<br>@endforeach</span>
@else
                    <input class="form-control" id="lname" type="text" name="lname" value="{{ old('lname') }}" placeholder="姓">
@endif
                    </div>
                    <!-- 名 -->
                    <div class="form-group">
...
@if ($errors->has('fname'))
                      <input class="form-control validate-error" id="fname" type="text" name="fname" value="{{ old('fname') }}" placeholder="名" aria-describedby="fname-error" aria-invalid="true">
                      <span id="fname-error" class="validate-error">@foreach ($errors->get('fname') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="fname" type="text" name="fname" value="{{ old('fname') }}" placeholder="名">
@endif
                    </div>
                    <!-- メールアドレス -->
                    <div class="form-group">
...
@if ($errors->has('email'))
                      <input class="form-control validate-error" id="email" type="email" name="email" value="{{ old('email') }}" placeholder="法人メールアドレスを入力してください" aria-describedby="email-error" aria-invalid="true">
                      <span id="email-error" class="validate-error">@foreach ($errors->get('email') as $message){{ $message }}<br>@endforeach</span>
@else
                      <input class="form-control" id="email" type="email" name="email" value="{{ old('email') }}" placeholder="法人メールアドレスを入力してください">
@endif
                    </div>
                    <!-- パスワード -->
                    <div class="form-group">
                      <label for="password">パスワード<strong class="text-danger">(必須)</strong></label>
...
@endif
                      <p class="help-block fz-12">※ 半角数字、半角英数(大文字、小文字)を8文字以上組み合わせてください</p>
                    </div>
                    <!-- パスワード確認 -->
                    <div class="form-group">
                      <label for="password-confirm">パスワード確認<strong class="text-danger">(必須)</strong></label>
...
@foreach (config('const.member.industry') as $value => $label)
                        <option value="{{ $value }}"{!! (int) old('industry') === $value ? ' selected="selected"' : '' !!}>{{ $label }}</option>
@endforeach
                      </select>
@if ($errors->has('industry'))
...
@foreach (config('const.member.location') as $value => $label)
                        <option value="{{ $value }}"{!! (int) old('location') === $value ? ' selected="selected"' : '' !!}>{{ $label }}</option>
@endforeach
                      </select>
@if ($errors->has('location'))
...
@foreach (config('const.member.employee') as $value => $label)
                        <option value="{{ $value }}"{!! (int) old('employee') === $value ? ' selected="selected"' : '' !!}>{{ $label }}</option>
@endforeach
                      </select>
@if ($errors->has('employee'))
...
@foreach (config('const.member.affiliation') as $value => $label)
                        <option value="{{ $value }}"{!! (int) old('affiliation') === $value ? ' selected="selected"' : '' !!}>{{ $label }}</option>
@endforeach
                      </select>
@if ($errors->has('affiliation'))
...
@foreach (config('const.member.handling') as $value => $label)
                      <div class="checkbox">
                        <label><input type="checkbox" name="handling[]" value="{{ $value }}"{!! is_array(old('handling')) && in_array($value, old('handling')) ? ' checked="checked"' : '' !!}>{{ $label }}</label>
                      </div>
@endforeach
...
@section('original_foot_js')
  ...
  <script src="/assets/js/members/new/validation-setting.js"></script>
  <!-- /add js -->
@endsection
